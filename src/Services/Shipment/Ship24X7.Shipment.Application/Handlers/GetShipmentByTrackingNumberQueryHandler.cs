...
/// <summary>
/// Handler for retrieving shipment details by tracking number with full related entities.
/// Validates customer ownership if customerId is provided for authorization.
/// </summary>
public class GetShipmentByTrackingNumberQueryHandler : IRequestHandler<GetShipmentByTrackingNumberQuery, ShipmentResponse?>
{
    ...
    public async Task<ShipmentResponse?> Handle(GetShipmentByTrackingNumberQuery request, CancellationToken cancellationToken)
    {
        var shipment = await _shipmentRepository.GetByTrackingNumberWithDetailsAsync(request.TrackingNumber);
        ...
        {
            throw new UnauthorizedAccessException("Customer does not own this shipment");
        }
